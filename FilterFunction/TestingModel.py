...
def train_model():
    ...
    joblib.dump(tfidf_vectorizer, 'premade_vectorizer.pkl')
...
def get_text(file_name):
    ...
    with open(file_name, "r") as file:
        data = json.load(file)
        final_reviews = []
        for star_rating, reviews in data.items():
...
def filter(reviews_json):
    ...
    tfidf_vectorizer = joblib.load('premade_vectorizer.pkl')
    ...
    filtered_reviews = [new_reviews[i] for i in range(len(new_reviews)) if predictions[i] == 1]
    print(f'Final size: {str(len(filtered_reviews))}')
